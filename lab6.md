...
### Задачи
    1. Создать базу данных в MongoDB на примере схемы, сделанной в первой лабораторной работе
...
1. Дополним `docker-compose.yaml`
```yaml
version: "3.9"
services:
  postgres:
  ...
  mongodb:
    ...
    ports:
      - "27017:27017"
    volumes:
      - ./sql/lab6:/docker-entrypoint-initdb.d
    environment:
      MONGO_INITDB_DATABASE: "pbd"
...
3. Добавим скрипт для вставки данных
```js
const db = db.getSiblingDB("pbd");
...
4. Добавим скрипт для создания `index` и `view`
```js
const db = db.getSiblingDB("pbd");
